...
const createOffer = async (req, res) => {
    try{
       ...
       const {title, poster, category, discount, product, startDate, endDate, duration} = req.body

       if(category === 'all' || !title  || discount === 0 || discount > 100 || product.length === 0 || !startDate || !endDate || duration === 0)
        ...
        const newOffer = new Offers({title: title.toLowerCase(), poster, discount, products: product, category, startDate, endDate, duration})

        await newOffer.save()

        res.json({
            msg: "Success! Created a new offer",
            newOffer
        })
...
const getOffer = async (req, res) => {
    try{
        ...
        const features = new APIfeatures(Offers.find(), req.query).filtering().sorting().paginating()
        const offer = await features.query

       res.json({success: 'Success', result: offer.length, offer})
    }
    catch(err) {
        return res.status(500).json({err: err.message})
    }
